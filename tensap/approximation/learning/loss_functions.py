...
class PoincareLossFunction(LossFunction):
    """
    ...
    @staticmethod
    def _eval(fun, sample, nargout=1):
        jac_g = fun(sample[0])
        jac_ut = np.moveaxis(sample[1], -1, -2)
        proj = np.array([jg.T @ np.linalg.solve(jg @ jg.T, jg) for jg in jac_g])
        residual = jac_ut - np.einsum("kil,klj->kij", proj, jac_ut)

        loss = np.linalg.norm(residual, axis=(1,2)) ** 2

        if nargout == 1:
            return loss
        loss_ref = np.linalg.norm(jac_ut, axis=(1,2)) ** 2
        return loss, loss_ref

    def eval(self, fun, sample, nargout=1):
        return self._eval(fun, sample, nargout)

    def relative_test_error(self, fun, sample, *args):
        risk, risk_ref = self.risk_estimation(fun, sample, nargout=2)
        return risk / risk_ref
...
class RayleighPoincareLossFunction(PoincareLossFunction):
    ...
    def build_jac_g(self, G):
        def jac_g(x):
            return np.einsum("ij, kjl -> kil", G.T, self.basis.eval_jacobian(x))
        return jac_g

    def eval(self, fun, sample, G, nargout=1):
        if fun is None:
            fun = self.build_jac_g(G)
        return self._eval(fun, sample, nargout)

    def eval_gradients(self, sample, G):
        m = G.shape[1]
        d = self.basis.ndim()
        N = len(sample[0])
        fun = self.build_jac_g(G)
        jac_g = fun(sample[0])
        jac_u = sample[1]
        jac_basis = self.basis.eval_jacobian(sample[0])
    ...
    def eval_gradient(self, sample, G):
        return self.eval_gradients(sample, G).mean(axis=0)
